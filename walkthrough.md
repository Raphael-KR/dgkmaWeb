...
- [x] 일반 페이지는 HTML·일반 텍스트만, 등록된 제공자 API는 JSON만 허용하고 압축 응답, 과대 응답, 지연된 헤더·본문과 과도한 리디렉션은 안전하게 중단한다.
...
- [x] JavaScript 렌더링은 등록된 정확한 부고 출처와 승인된 데이터 호스트만 사용하고, 모든 DNS 응답 검증·주소 고정·미등록 호스트 차단을 적용한다.
- [x] 등록된 제공자 API는 정적 HTML과 JavaScript보다 먼저 사용하고, 최소 응답 스키마 검증 또는 API 요청이 실패하면 기존 페이지 렌더링과 문자 분석으로 fallback한다.
- [x] 출처 성공 응답은 `provider-api`, `static-html`, `javascript` 처리 방법을 구분하며 본인상은 동문 이름과 고인 이름이 다르면 미리보기와 게시를 차단한다.
...
Replit 서버 코어에서 같은 링크와 `졸업21기 OOO` 힌트를 다시 분석한 결과 출처는 `fetched`, 고인·78세·발인·빈소·관계는 모두 추출되고 필수 누락은 0건이었다. 실페이지의 `(남/78세)` 표기를 처음 fixture가 반영하지 못해 나이·관계가 누락된 두 번째 RED를 확인했고, 해당 형식을 회귀 테스트로 고정한 뒤 실제 링크를 다시 통과시켰다. 최종 Replit 집중 테스트 32/32, 전체 366/366, `npm run check`, `npm run build`, `git diff --check`가 통과했다. 이 단계는 서버 수집·파싱 검증이며 로그인 회원 화면의 초안·미리보기·게시·목록·상세 검증이나 Production Republish 완료를 뜻하지 않는다.

같은 날 기품 공개 페이지가 사용하는 인증 없는 구조화 API를 제공자 전용 빠른 경로로 추가했다. API 요청은 기존 공개 주소 검증·DNS 고정·512 KiB·timeout·비압축 정책을 그대로 사용하고 `application/json`만 허용하며, 필요한 고인·관계 근거·발인·빈소·연락처·계좌 필드만 스키마 검증해 공통 부고 파서에 전달한다. API가 실패하면 기존 Chromium으로 fallback하고, 본인상에서 동문 이름과 고인 이름이 다르면 미리보기·게시를 차단한다. 합성 응답만 사용하는 Replit 집중 테스트 58/58, 전체 `npm test`, `npm run check`, `npm run build`, `git diff --check`가 통과했다. 실제 링크의 개인정보 값을 출력하지 않는 서버 smoke에서는 출처가 `fetched`·처리 방법이 `provider-api`이고 필수 누락 0건이며 관련 동문·고인·나이·관계·발인·빈소·계좌·연락처가 모두 존재함을 확인했다. 인앱 브라우저 연결을 사용할 수 없어 로그인 회원 화면의 초안·미리보기·게시·목록·상세는 이 단계에서 확인하지 않았다.

2026-08-16 `kakaobugo.com/m/39716`은 HTTP `200`이지만 공개 원문이 `삭제된 부고입니다` 안내와 PHP include 경고·외부 서버 경로만 반환했다. 기존 정제기는 삭제 안내가 든 `script`·`noscript`를 제거한 뒤 PHP 경고만 정상 본문으로 취급해 출처를 `fetched`로 오판했다. 실제 형태의 회귀 테스트를 먼저 추가해 RED를 확인하고, 삭제·종료 안내와 PHP include 경고를 `unavailable`로 분류해 외부 오류와 경로가 결합 본문이나 응답에 남지 않도록 수정했다. 함께 입력한 `1기 OOO 본인상`은 졸업기수 축약을 관련 동문 근거로 인정하고 본인상 고인 동일성을 적용하되, 원문과 문자에 없는 향년·발인·빈소는 계속 누락으로 반환한다. 수정 후 Replit SSH 집중 테스트 32/32와 실제 링크 smoke에서 `unavailable`, 문자 fallback 유지, 관련 동문·본인 관계·고인명 확인, 향년·발인·빈소 누락을 확인했다. 필수 정보가 부족해 초안 게시·목록·상세와 DB 쓰기는 수행하지 않았다.
