...
async function insertOperation(client: PoolClient, input: { command: LegacyPaymentCommand | LegacyPaymentReadTransitionCommand; actor: SourceDecisionActor; receipt: LegacyPaymentReceipt; rootCorrelationUid: string; results: Json[]; slots: Json[]; receiptId: string }): Promise<void> {
  const slots=[...input.slots].sort((left,right)=>Number(left.phase)-Number(right.phase)||Number(left.result_ordinal)-Number(right.result_ordinal)||String(left.qualified_table_name).localeCompare(String(right.qualified_table_name))||Number(left.local_ordinal)-Number(right.local_ordinal));
  const payload = { command: `legacy_payment:${input.command.action}`, expected_results: input.results, inputs: { command_sha256: input.receipt.command_sha256, operation_receipt: input.receipt as unknown as CanonicalValue }, reservation_slots: slots, schema_version: "business-operation-payload-v2" };
  const validatedPayload=validateBusinessOperationPayloadV2(payload as unknown as CanonicalValue);
  await client.query(`INSERT INTO public.business_operation_receipts (id,action,actor_name_snapshot,actor_scope,actor_target_user_id,actor_target_user_id_snapshot,actor_uid_snapshot,actor_user_id,actor_user_id_snapshot,authorization_version,canonical_payload,entity_type,operation_uid,payload_sha256,recorded_at,result_entity_keys,root_correlation_uid,target_fingerprint) OVERRIDING SYSTEM VALUE VALUES ($1,$2,$3,'migration_admin',NULL,NULL,$4::uuid,$5,$5,$6,$7::jsonb,'legacy_payment',$8::uuid,$9,$10::timestamptz,$11::jsonb,$12::uuid,$13)`, [input.receiptId, input.command.action, input.actor.name, input.actor.userUid, input.actor.userId, input.actor.authorizationVersion, validatedPayload.canonical, input.command.operationUid, validatedPayload.sha256, input.receipt.recorded_at, canonicalJson(input.results as unknown as CanonicalValue), input.rootCorrelationUid, input.actor.targetFingerprint]);
