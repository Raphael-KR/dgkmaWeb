...
export async function executeLegacyPaymentCommand(pool: Pick<Pool, "connect">, rawCommand: Record<string, unknown>, actor: SourceDecisionActor): Promise<LegacyPaymentExecution> {
  ...
  try {
    ...
    const ledger = await client.query<{ count: number }>("SELECT count(*)::int count FROM public.schema_change_ledger WHERE sequence_no=90 AND manifest_sha256=$1", ["19ac53ce74af375ab5eb6a9c96a3ca4d5fd7cc5127e9ad7bbd1da4cad33ea700"]); if (ledger.rows[0]?.count !== 1) fail("legacy_payment_sequence_90_missing");
    const source = await client.query<{ id: string }>("SELECT id::text FROM public.accounting_logical_sources WHERE source_code='LEGACY_PAYMENTS' FOR UPDATE"); if (source.rowCount !== 1) fail("legacy_payment_source_missing");
    ...
    if (command.action === "preview") {
      await client.query("LOCK TABLE public.payments IN SHARE MODE"); const payments = await client.query<{ count: number }>("SELECT count(*)::int count FROM public.payments"); if (payments.rows[0].count !== 0) fail("legacy_payment_nonzero_materialization_not_implemented");
      const batchId = await reserve(client, "accounting_import_batches"); const auditId = await reserve(client, "accounting_audit_events"); const result = { ordinal: 1, entity_type: "import_batch", entity_key: `batch:${batch.batch_uid}`, entity_action: "preview", action_correlation_uid: selected.action_correlation_uid } as Json; results.push(result); slots.push({ local_ordinal: 1, phase: 10, qualified_table_name: "public.accounting_import_batches", reserved_id: batchId, result_ordinal: 1, slot_kind: "business_row", slot_kind_order: 10 }, { local_ordinal: 1, phase: 30, qualified_table_name: "public.accounting_audit_events", reserved_id: auditId, result_ordinal: 1, slot_kind: "audit_row", slot_kind_order: 30 });
      const receipt = hashedReceipt({ schema_version: "dgkma-legacy-payment-operation-v1", action: command.action, operation_uid: command.operationUid, plan_sha256: String(plan.plan_sha256), committed_outcome: "created", phase, watermark_payment_id: watermark, comparison_digest: comparisonDigest, source_release_uid: String(release.release_uid), batch_uid: String(batch.batch_uid), source_fingerprint: String(batch.source_fingerprint), actor_user_id: actor.userId, actor_user_uid: actor.userUid, authorization_version: actor.authorizationVersion, target_fingerprint: actor.targetFingerprint, recorded_at: at, command_sha256: commandSha });
      await insertOperation(client, { command, actor, receipt, rootCorrelationUid: String(selected.root_correlation_uid), results, slots, receiptId });
      await client.query(`INSERT INTO public.accounting_import_batches (id,batch_uid,source_release_id,source_fingerprint,captured_timezone,coverage_from,coverage_through,source_revision,preview_manifest,preview_manifest_sha256,status,previewed_at,preview_actor_at,preview_actor_authorization_version,preview_actor_correlation_uid,preview_actor_name_snapshot,preview_actor_scope,preview_actor_uid_snapshot,preview_actor_user_id,row_count,warning_count,error_count) OVERRIDING SYSTEM VALUE VALUES ($1,$2::uuid,$3,$4,'Asia/Seoul',$5::timestamptz,$6::timestamptz,$7,'[]'::jsonb,$8,'previewed',$9::timestamptz,$9::timestamptz,$10,$11::uuid,$12,'admin',$13::uuid,$14,0,0,0)`, [batchId, batch.batch_uid, releaseRow.rows[0].id, batch.source_fingerprint, batch.coverage_from, batch.coverage_through, plan.source_revision, batch.preview_manifest_sha256, at, actor.authorizationVersion, selected.action_correlation_uid, actor.name, actor.userUid, actor.userId]);
      await audit(client, auditId, String(selected.audit_event_uid), result, actor, at, null, { row_count: 0, source_fingerprint: batch.source_fingerprint }, "TODO19_LEGACY_ZERO_ROW_PREVIEW"); await client.query("COMMIT"); return { execution_outcome: "created", receipt };
    }
