...
export function buildGroupMaterializationTopology(plan: GroupMultiBatchApplyPlan): GroupMaterializationStep[] {
  if (!plan.resolvedBindings || plan.groups.length === 0 || plan.groups.some((group) => !group.primaryEvidence?.normalizationVersion || !/^[0-9a-f]{64}$/.test(group.classificationDecisionPayloadSha256) || !["dues","mixed"].includes(group.classificationKind) || !Number.isInteger(group.duesYear) || group.duesYear<=0 || group.allocations.some((allocation) => !allocation.evidence?.normalizationVersion || typeof allocation.evidence.normalizedPayload.member_name_key_digest !== "string" || !/^[0-9a-f]{64}$/.test(allocation.evidence.normalizedPayload.member_name_key_digest) || !Number.isInteger(allocation.evidence.normalizedPayload.generation) || !allocation.evidenceKind || !/^[0-9a-f]{64}$/.test(allocation.evidenceDigest) || !allocation.scoreBasis))) fail("source_decision_group_materialization_evidence_missing");
