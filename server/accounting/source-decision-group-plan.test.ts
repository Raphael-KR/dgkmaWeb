...
test("loads and locks the exact live companion graph before planning", async () => {
  ...
  const client = { query: async (text: string) => {
    ...
    if (text.includes("FROM public.accounting_categories")) return { rowCount: 1, rows: [{ id: "101", category_code: "DUES_INCOME", active_from: "2025-01-01", active_to: null, dues_effect: "dues_credit" }] };
