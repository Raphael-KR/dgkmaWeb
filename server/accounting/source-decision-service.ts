...
export async function decideSourcePreview(
  ...
  try {
    ...
    if (command.decision === "approve") for (const item of itemResult.rows) {
      ...
      if (typeof periodCode !== "string" || !periodCode || typeof startsAt !== "string" || !Number.isFinite(Date.parse(startsAt)) || endsAt !== null && (typeof endsAt !== "string" || !Number.isFinite(Date.parse(endsAt))) || typeof boundarySourceCode !== "string" || typeof boundaryCoordinateKey !== "string" || typeof boundaryContentDigest !== "string" || !/^[0-9a-f]{64}$/.test(boundaryContentDigest) || endsAt !== null && Date.parse(startsAt) >= Date.parse(endsAt)) fail("source_decision_period_payload_invalid");
      const boundary = await client.query<{ id: string }>(`SELECT rv.id::text FROM public.accounting_import_row_versions rv JOIN public.accounting_import_coordinates c ON c.id=rv.coordinate_id JOIN public.accounting_logical_sources ls ON ls.id=c.logical_source_id WHERE ls.source_code=$1 AND c.coordinate_key=$2 AND rv.content_digest=$3 FOR UPDATE OF rv,c,ls`, [boundarySourceCode,boundaryCoordinateKey,boundaryContentDigest]);
