...
test("community event composer progressively reveals schema-backed review fields", async () => {
  ...
  assert.match(composer, /apiRequest\(\s*"POST",\s*"\/api\/events\/parse"/);
  assert.match(composer, /eventType: snapshot\.eventType[\s\S]*input: snapshotSourceText/);
  assert.match(composer, /mergeParsedEventDraft/);
  assert.match(composer, /setSourceStatuses/);
  assert.match(composer, /링크 내용을 불러왔습니다\./);
  assert.match(composer, /링크가 종료되었거나 공개되지 않아 입력한 문자만 분석했습니다\./);
  assert.match(composer, /안전하지 않은 주소는 읽지 않았습니다\./);
  assert.match(composer, /parseMissingFields/);
  assert.doesNotMatch(composer, /\/api\/obituary\/parse/);
  assert.doesNotMatch(composer, /링크 내용 수집은 준비 중/);
