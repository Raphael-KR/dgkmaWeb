...
test("legacy payment POST uses the same frozen-admin application service contract as the CLI", async () => {
  const frozen = { userId: 7, userUid: "77777777-7777-4777-8777-777777777777", authorizationVersion: "8".repeat(64), targetFingerprint: "9".repeat(64) }; let writes = 0;
  const command = { schemaVersion: "legacy-payment-command-v1", action: "fence", operationUid: "12345678-1234-4234-8234-123456789abc", planSha256: "a".repeat(64), expectedPhase: "legacy", expectedWatermarkPaymentId: 0, expectedComparisonDigest: "b".repeat(64) };
  const execution = { execution_outcome: "created" as const, receipt: { schema_version: "dgkma-legacy-payment-operation-v1" as const, action: "fence" as const, operation_uid: command.operationUid, plan_sha256: command.planSha256, committed_outcome: "created" as const, phase: "fenced" as const, watermark_payment_id: 0, comparison_digest: command.expectedComparisonDigest, source_release_uid: "11111111-1111-4111-8111-111111111111", batch_uid: "22222222-2222-4222-8222-222222222222", source_fingerprint: "c".repeat(64), actor_user_id: 7, actor_user_uid: frozen.userUid, authorization_version: frozen.authorizationVersion, target_fingerprint: frozen.targetFingerprint, recorded_at: "2026-08-11T00:00:00.000Z", command_sha256: "d".repeat(64), receipt_sha256: "e".repeat(64) } };
  const app = express(); app.use((req, _res, next) => { (req as any).session = { userId: Number(req.get("x-test-user-id")) || undefined }; next(); });
  const server = await registerRoutes(app, { getUserForAdmin: async (id) => id ? { isAdmin: true } : undefined, getSourceDecisionAdmin: async (id) => ({ id, userUid: frozen.userUid, name: "관리자", isAdmin: true }), sourceDecisionAdminReceipt: frozen, legacyPaymentExecutor: async (received, actor) => { writes += 1; assert.deepEqual(received, command); assert.equal(actor.authorizationVersion, frozen.authorizationVersion); return execution; } });
  await new Promise<void>((resolve) => server.listen(0, "127.0.0.1", resolve)); const port = (server.address() as AddressInfo).port; const base = `http://127.0.0.1:${port}`; const path = "/api/admin/accounting/legacy-payments/command";
  try {
    const crossOrigin = await fetch(`${base}${path}`, { method: "POST", headers: { "content-type": "application/json", "x-test-user-id": "7", origin: "https://evil.example", "sec-fetch-site": "same-origin" }, body: JSON.stringify(command) }); assert.equal(crossOrigin.status, 403); assert.equal(writes, 0);
    const response = await fetch(`${base}${path}`, { method: "POST", headers: { "content-type": "application/json", "x-test-user-id": "7", origin: base, "sec-fetch-site": "same-origin" }, body: JSON.stringify(command) }); assert.equal(response.status, 200); assert.deepEqual(await response.json(), execution); assert.equal(writes, 1);
  } finally { await new Promise<void>((resolve, reject) => server.close((error) => error ? reject(error) : resolve())); }
});
