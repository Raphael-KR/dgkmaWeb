...
- [진행 중, 코드·Replit 검증] `/events`의 항상 표시 등록 도구, `전체`/유형별 목록 필터, 유형별 필드·필수 오류, 텍스트 전용 파싱 안내, 초안 자동저장·복구·삭제·재시도, 서버 데이터 기반 부고 미리보기와 복사
- [진행 중, 코드·Replit 검증] 게시 직후 경조사 목록 갱신, 게시판·회원 홈 진입점, `/o`·`/o/new` legacy 리디렉션과 `/o/:id` legacy 상세 유지
- [진행 중, 카카오 심사 승인] 카카오 동의항목 심사 가이드, 개인정보 없는 운영 로그인 화면 캡처와 심사용 PDF
...
경조사 통합 기반과 등록·초안 UI는 코드·Replit 검증 단계이며, 마이그레이션 SQL은 자동 실행되지 않는다. 카카오 동의·탈퇴 변경은 현재 HEAD 전체 Replit 자동화, 실제 개발 OAuth·명부 매칭·탈퇴, 운영 로그인 화면 캡처, 심사용 PDF, 운영 additive 스키마·Republish·비로그인 smoke·국외이전 고지, 카카오 개인정보 동의항목 권한 승인과 콘솔 동의 단계 설정까지 확인했다. 운영 OAuth·탈퇴는 아직 남아 있다. 기존 부고 기본 기능의 프로덕션 검증 이력과 공개 `/about/condolence`는 유지한다.
