...
- 일부 공개 부고 사이트가 식별자 없는 서버 요청에 `500`을 반환하거나 ASP.NET 폼 내부의 표시 본문이 제거돼 정상 링크를 만료 상태로 오판하던 문제를 고정 파서 `User-Agent`와 표시 콘텐츠 중심 정제로 수정하고, `향년` 없는 나이와 줄바꿈형 상주 정보를 정확히 추출
