...
- [진행 중, 코드·Replit 검증] `/events`의 간결한 입력과 점진적 확인 영역, 네 유형별 등록 명령, 목록 필터 없는 전체 경조사 목록, 유형별 필드·필수 오류, 텍스트 전용 파싱 안내, 초안 자동저장·복구·삭제·재시도, 서버 데이터 기반 부고 미리보기와 복사
- [진행 중, 코드·Replit 검증] 게시 직후 경조사 목록 갱신, 게시판·회원 홈 진입점, `/o`·`/o/new` legacy 리디렉션과 `/o/:id` legacy 상세 유지
- [진행 중, 로컬 자동화 검증] 회원 전용 경조사 파싱 API, 문자·공개 링크 혼합 입력, 출처 상태와 부고 필수 정보 누락 안내
