...
경조사 통합 기반과 등록·초안 UI는 코드·Replit 검증 단계이며, 마이그레이션 SQL은 자동 실행되지 않는다. 실제 회원 브라우저 검증, Republish, 개발·프로덕션 DB 마이그레이션, 공개 링크 내용 수집은 완료되지 않았다. 기존 부고 기본 기능의 프로덕션 검증 이력과 공개 `/about/condolence`는 유지한다.
