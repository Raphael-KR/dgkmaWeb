...
- [진행 중, 자동화 재검증·실제 개발 smoke 대기] 카카오 동의항목 심사와 회원 탈퇴 운영 가이드
...
경조사 통합 기반과 등록·초안 UI는 코드·Replit 검증 단계이며, 마이그레이션 SQL은 자동 실행되지 않는다. 카카오 동의·탈퇴 변경의 현재 HEAD 전체 Replit 자동화 검증, 실제 브라우저 스크린샷, 실제 OAuth·탈퇴 smoke, 심사용 PDF, Republish는 완료되지 않았다. 기존 부고 기본 기능의 프로덕션 검증 이력과 공개 `/about/condolence`는 유지한다.
