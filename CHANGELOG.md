...
- 검증된 공개 부고 제공자 API와 완전한 정적 부고 HTML을 우선하는 서버 파서, 실제 가족관계 근거 기반 관계 추론, 공식 개명 별칭 매칭
...
- 제공자 API와 정적 HTML 요청에 기존 공개 주소 검증·DNS 주소 고정·응답 제한을 적용하고, 불완전한 앱 셸은 성공으로 취급하지 않으며 서버의 제3자 JavaScript 실행 경로를 제거
