...
- [진행 중] 경조사 통합 기반의 타입 계약, 저장 모델, 회원 전용 API와 부고 마이그레이션 SQL 스크립트
...
경조사 통합 기반은 코드와 검증 테스트까지 진행 중이며, 마이그레이션 SQL은 자동 실행되지 않는다. UI, 문자·링크 파싱, 초안 복구 UI와 개발·프로덕션 DB 마이그레이션은 아직 완료되지 않았다.
