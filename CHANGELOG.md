...
- [개발 검증 중] 초기 HTML에 본문이 없는 허용된 공개 부고 제공자를 Chromium으로 제한 렌더링하는 서버 파서와 실제 가족관계 근거 기반 관계 추론
...
- [코드·Replit 및 운영 실제 계정 검증 완료] 환경별 카카오 회원번호 서버 Secret allowlist와 로그인 시 관리자 권한 자동 복구
- [운영 실제 계정 검증 완료] 커뮤니티 홈의 관리자 상태 배지를 관리자 패널 진입 링크로 변경하고 전체 페이지 재요청 뒤 관리자 세션 유지 확인
...
- [Development 실제 계정 검증 완료] PR #11 병합본의 문자+공개 링크 수집, 관련 동문·관계·제목·날짜 병합, 초안 자동저장·삭제와 화면 초기화
...
- 일반회원은 본인 경조사만 등록하고, 관리자는 입력한 이름과 학번이 명부에서 정확히 한 명과 일치할 때만 해당 동문의 경조사를 대리 등록하도록 정책과 서버 대상자 판정을 일치

### Fixed

- 일부 공개 부고 사이트가 식별자 없는 서버 요청에 `500`을 반환하거나 ASP.NET 폼 내부의 표시 본문이 제거돼 정상 링크를 만료 상태로 오판하던 문제를 고정 파서 `User-Agent`와 표시 콘텐츠 중심 정제로 수정하고, `향년` 없는 나이·줄바꿈형 상주·분리된 빈소 호실을 정확히 추출하며 계좌 선택 안내문을 계좌번호로 저장하지 않도록 보강
- 날짜가 포함된 `OOO동문의 부친께서 YYYY년 M월 D일 별세` 문장에서 관계와 관련 동문을 놓치던 부고 문자 파서를 보강
- 관리자가 다른 동문의 부고를 작성할 때 표준 미리보기와 게시 값이 대상 동문이 아닌 작성자 회원정보로 바뀌던 문제를 이름+학번 단일 명부 일치 대상자의 서버 정보로 정규화
- 프로젝트 지침 문구를 간결하게 바꿔도 동일한 카카오 어드민 키 범위를 인정하도록 문서 계약 회귀를 의미 기반으로 보강
...
- JavaScript 부고 렌더링을 정확한 제공자 도메인과 승인된 데이터 호스트의 HTTPS 기본 포트로 제한하고, 모든 DNS 응답의 공개 주소 검증·주소 고정·그 밖의 호스트 해석 차단을 적용
- 성공한 카카오 회원 로그인에서 검증·회원 확정 뒤 세션 식별자를 재생성하고 새 세션에만 `userId`를 저장해, 로그인 전 세션이 인증 상태를 승계하지 못하게 수정
...
- 공개 Development URL의 디버그 로그인 우회와 클라이언트 query 우회를 제거
...
- [모바일 QA 수정] 공개 홈페이지 회원 구성 표가 390px 화면에서 grid 폭을 넓혀 가로 스크롤을 만드는 문제를 수정
