...
export const STARTUP_MANIFEST_SHA256 = "ec18e2c2a60ee9fed75ac97bd77f56b281804313de624cf7d18bce0f43a86b1e";
export const STARTUP_EXECUTOR_VERSION = "schema-ledger-v1";
export const STARTUP_REQUIRED_SEQUENCES = [1, 10, 15, 20, 30, 40, 50, 60, 70, 80, 90, 100, 110, 120, 130] as const;
export const STARTUP_LATEST_REQUIRED = { sequence_no: 130, artifact_id: "schema-exception-registry-enforcement-v1" } as const;
...
export const STARTUP_ARTIFACTS = [
  ...
  { sequence_no: 110, artifact_id: "business-reason-registry-v1", kind: "manual" },
  { sequence_no: 120, artifact_id: "business-reason-transition-enforcement-v1", kind: "manual" },
  { sequence_no: 130, artifact_id: "schema-exception-registry-enforcement-v1", kind: "manual" },
] as const;
...
export function verifyStartupLedger(rows: readonly StartupLedgerRow[], descriptors: readonly StartupDescriptor[]) {
  ...
  if (rows.some((row) => row.sequence_no <= 130 && !STARTUP_REQUIRED_SEQUENCES.includes(row.sequence_no as never))) {
...
export function startupLedgerInventorySql(): string {
  return [
    ...
    "WHERE l.sequence_no <= 130 ORDER BY l.sequence_no, l.artifact_id;",
    "ROLLBACK;",
    "",
  ].join("\n");
}
