...
function main(): void {
  ...
  const exceptionRegistry=value.schema_exception_registry;
  if(!Array.isArray(exceptionRegistry?.rules)||exceptionRegistry.rules.length!==25||new Set(exceptionRegistry.rules.map((rule:any)=>rule.rule_code)).size!==25||new Set(exceptionRegistry.rules.map((rule:any)=>rule.predicate_sql)).size!==25||exceptionRegistry.rules.filter((rule:any)=>rule.exception_class==="pre_anchor_blocking").length!==21||exceptionRegistry.rules.filter((rule:any)=>rule.exception_class==="legacy_not_valid").length!==4)fail("manifest_schema_exception_registry_closure_mismatch");
  if(exceptionRegistry.rules[0]?.rule_code!=="USERS_EMAIL_CANONICAL_BLANK"||exceptionRegistry.rules.some((rule:any)=>!/^[A-Z][A-Z0-9_]+$/.test(rule.rule_code)||typeof rule.duplicate_class!=="boolean")||JSON.stringify(exceptionRegistry.statuses)!=='["open","resolved","waived"]'||JSON.stringify(exceptionRegistry.resolutions)!=='["SOURCE_FIXED","DUPLICATE_RESOLVED","OWNER_WAIVER"]'||exceptionRegistry.open_resolution!==null||exceptionRegistry.post_capture_root_insert!=="rejected"||exceptionRegistry.transition_function!=="public.dgkma_validate_schema_exception_transition_v1()"||exceptionRegistry.rejection_sqlstate!=="23514")fail("manifest_schema_exception_registry_contract_mismatch");
  ...
  const greatGreatGreatGreatGrandparentManifest=readManifest((greatGreatGreatGrandparentManifest.value.manifest_lineage as {parent_manifest_path:string}).parent_manifest_path);
  if (sequence70.length !== 1 || !sequence70[0].required_for_startup || !sequence70[0].required_for_production || sequence70[0].manifest_sha256 !== greatGreatGreatGreatGrandparentManifest.sha256) fail("manifest_sequence_70_route_mismatch");
  if(sequence80.length!==1||!sequence80[0].required_for_startup||!sequence80[0].required_for_production||sequence80[0].manifest_sha256!==greatGreatGreatGrandparentManifest.sha256)fail("manifest_sequence_80_route_mismatch");
  if(sequence90.length!==1||!sequence90[0].required_for_startup||!sequence90[0].required_for_production||sequence90[0].manifest_sha256!==greatGreatGrandparentManifest.sha256)fail("manifest_sequence_90_route_mismatch");
  if(sequence100.length!==1||!sequence100[0].required_for_startup||!sequence100[0].required_for_production||sequence100[0].manifest_sha256!==greatGrandparentManifest.sha256)fail("manifest_sequence_100_route_mismatch");
  if(sequence110.length!==1||!sequence110[0].required_for_startup||!sequence110[0].required_for_production||sequence110[0].manifest_sha256!==grandparentManifest.sha256)fail("manifest_sequence_110_route_mismatch");
  if(sequence120.length!==1||!sequence120[0].required_for_startup||!sequence120[0].required_for_production||sequence120[0].manifest_sha256!==parentManifest.sha256)fail("manifest_sequence_120_route_mismatch");
  if(sequence130.length!==1||!sequence130[0].required_for_startup||!sequence130[0].required_for_production||sequence130[0].manifest_sha256!==manifest.sha256)fail("manifest_sequence_130_route_mismatch");
