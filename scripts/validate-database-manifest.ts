...
function main(): void {
  ...
  const sequence70 = descriptors.filter((entry) => entry.sequence_no === 70);const sequence80=descriptors.filter((entry)=>entry.sequence_no===80);const sequence90=descriptors.filter((entry)=>entry.sequence_no===90);const sequence100=descriptors.filter((entry)=>entry.sequence_no===100);
  ...
  if (sequence70.length !== 1 || !sequence70[0].required_for_startup || !sequence70[0].required_for_production || sequence70[0].manifest_sha256 !== greatGrandparentManifest.sha256) fail("manifest_sequence_70_route_mismatch");
  if(sequence80.length!==1||!sequence80[0].required_for_startup||!sequence80[0].required_for_production||sequence80[0].manifest_sha256!==grandparentManifest.sha256)fail("manifest_sequence_80_route_mismatch");
  if(sequence90.length!==1||!sequence90[0].required_for_startup||!sequence90[0].required_for_production||sequence90[0].manifest_sha256!==parentManifest.sha256)fail("manifest_sequence_90_route_mismatch");
  if(sequence100.length!==1||!sequence100[0].required_for_startup||!sequence100[0].required_for_production||sequence100[0].manifest_sha256!==manifest.sha256)fail("manifest_sequence_100_route_mismatch");
