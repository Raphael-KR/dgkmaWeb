...
const SEQUENCES = [1, 10, 15, 20, 30, 40, 50, 60, 65, 70, 80, 90, 100, 110];
const EXPECTED_KINDS = new Map<number, ArtifactDescriptor["kind"]>([
  [1,"manual"],[10,"baseline"],[15,"ordinary"],[20,"ordinary"],[30,"ordinary"],[40,"manual"],[50,"ordinary"],[60,"manual"],[65,"ordinary"],[70,"manual"],[80,"manual"],[90,"manual"],[100,"manual"],[110,"manual"],
]);
...
export function readArtifactDescriptors(directory = "migrations/artifacts", manifestPath = "docs/database-manifest.yaml"): ArtifactDescriptor[] {
  ...
  if (!lineage || typeof lineage.parent_manifest_sha256 !== "string" || !SHA.test(lineage.parent_manifest_sha256) || typeof lineage.parent_manifest_path !== "string" || lineage.amendment_code !== "business_reason_registry_v1") fail("manifest_lineage_invalid");
  ...
  if(!parentLineage||typeof parentLineage.parent_manifest_sha256!=="string"||!SHA.test(parentLineage.parent_manifest_sha256)||typeof parentLineage.parent_manifest_path!=="string"||parentLineage.amendment_code!=="legacy_payments_write_fence_v1")fail("manifest_parent_lineage_invalid");
  ...
  if(!grandparentLineage||typeof grandparentLineage.parent_manifest_sha256!=="string"||!SHA.test(grandparentLineage.parent_manifest_sha256)||typeof grandparentLineage.parent_manifest_path!=="string"||grandparentLineage.amendment_code!=="legacy_cutover_code_root_v1")fail("manifest_grandparent_lineage_invalid");
  ...
  if(!greatGrandparentLineage||typeof greatGrandparentLineage.parent_manifest_sha256!=="string"||!SHA.test(greatGrandparentLineage.parent_manifest_sha256)||typeof greatGrandparentLineage.parent_manifest_path!=="string"||greatGrandparentLineage.amendment_code!=="group_member_source_root_v1")fail("manifest_great_grandparent_lineage_invalid");
  ...
  if(!greatGreatGrandparentLineage||typeof greatGreatGrandparentLineage.parent_manifest_sha256!=="string"||!SHA.test(greatGreatGrandparentLineage.parent_manifest_sha256)||typeof greatGreatGrandparentLineage.parent_manifest_path!=="string"||greatGreatGrandparentLineage.amendment_code!=="event_claim_coordinate_root_v1")fail("manifest_great_great_grandparent_lineage_invalid");
  const rootManifest=readManifest(greatGreatGrandparentLineage.parent_manifest_path);if(rootManifest.sha256!==greatGreatGrandparentLineage.parent_manifest_sha256)fail("manifest_root_checksum_mismatch");
  ...
  if (descriptors.length !== 15) fail("artifact_descriptor_count_mismatch");
  ...
  for (const descriptor of descriptors) {
    ...
    const expectedManifestSha = descriptor.sequence_no === 110 ? manifest.sha256 : descriptor.sequence_no === 100 ? parentManifest.sha256 : descriptor.sequence_no === 90 ? grandparentManifest.sha256 : descriptor.sequence_no === 80 ? greatGrandparentManifest.sha256 : descriptor.sequence_no === 70 ? greatGreatGrandparentManifest.sha256 : rootManifest.sha256;
...
export async function applyArtifact(
  ...
  try {
    ...
    if (descriptor.sequence_no === 70 || descriptor.sequence_no === 80 || descriptor.sequence_no === 90 || descriptor.sequence_no === 100 || descriptor.sequence_no === 110) await pool.query(readFileSync(descriptor.path, "utf8"));
