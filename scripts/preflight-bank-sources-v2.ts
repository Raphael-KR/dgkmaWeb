...
async function main() {
  ...
  for (const source of SOURCES) {
    ...
    const rows = Array.from({ length }, (_, index) => ({ rowNumber: selector.data_start_row_or_null + index, values: Object.fromEntries(selected.map((name) => [name, values[name][index] ?? null])) })).filter((row) => Object.values(row.values).some((value) => text(value) !== ""));
    let dateNumbers = 0; let dateStrings = 0; let amountValid = 0; let balanceValid = 0; let exclusiveDirection = 0; let displayPresent = 0; const anomalyRows: Array<{ row_number: number; present_allowed_headers: string[] }> = [];
    for (const sourceRow of rows) {
      const row = sourceRow.values; let validTransaction = false; let hasDisplay = false;
      const date = row[source.code === "BANK_TOSS_2026" ? "거래 일시" : "거래일시"]; if (typeof date === "number") dateNumbers += 1; else if (text(date)) dateStrings += 1;
      if (source.code === "BANK_TOSS_2026") { const amount = money(row["거래 금액"]); if (amount !== null && amount !== 0) { amountValid += 1; exclusiveDirection += 1; validTransaction = true; } if (money(row["거래 후 잔액"]) !== null) balanceValid += 1; hasDisplay = ["적요", "거래 유형", "거래 기관", "메모"].some((name) => text(row[name])); if (hasDisplay) displayPresent += 1; }
      else { const debit = money(row["출금"]); const credit = money(row["입금"]); if ((debit !== null && debit !== 0) !== (credit !== null && credit !== 0)) { amountValid += 1; exclusiveDirection += 1; validTransaction = true; } if (money(row["거래후 잔액"]) !== null) balanceValid += 1; hasDisplay = ["상대계좌예금주명", "거래내용", "상대은행", "메모"].some((name) => text(row[name])); if (hasDisplay) displayPresent += 1; }
      if (!validTransaction || !hasDisplay) anomalyRows.push({ row_number: sourceRow.rowNumber, present_allowed_headers: selected.filter((name) => text(row[name]) !== "") });
    }
