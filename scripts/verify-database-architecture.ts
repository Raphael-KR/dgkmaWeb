...
function runTaskSixteen(): void {
  ...
  for (const variant of variants) {
    ...
    summaries.push({ run_uid:runUid, requested_variant:variant, ledger_sequences:[1,10,15,20,30,40,50,60,70,80,90], reapply:"verified_noop", teardown_absent:true });
  }
  ...
  writeFileSync(evidencePath, `${canonicalJson({ schema_version:"dgkma-task-evidence-v1",task:16,case:caseName,
    ...
    assertions:{runs:summaries,artifact_descriptors:descriptors.length,startup_required_through:90,production_operations:0,development_schema_writes:0},
