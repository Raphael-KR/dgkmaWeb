...
const DESCRIPTOR_PATH = "docs/source-contracts/releases/notion-organization-role-history-v3.json";
...
async function reserve(client: PoolClient, table: string): Promise<string> { const result = await client.query<{ id: string }>("SELECT nextval(pg_get_serial_sequence($1,'id'))::text id", [`public.${table}`]); return result.rows[0].id; }

async function register(client: PoolClient, targetFingerprint: string) {
  const plan = readJson(PLAN_PATH); const planPreimage = { ...plan }; delete planPreimage.plan_sha256;
  if (plan.plan_sha256 !== sha256(canonicalJson(planPreimage as CanonicalValue)) || plan.target_fingerprint !== targetFingerprint || plan.source_code !== "NOTION_ORGANIZATION_ROLE_HISTORY") fail("notion_role_v3_plan_binding_mismatch");
  const descriptorBytes = readFileSync(DESCRIPTOR_PATH); const descriptor = JSON.parse(descriptorBytes.toString("utf8")) as Json;
  if (plan.descriptor_sha256 !== sha256(descriptorBytes) || descriptor.source_code !== plan.source_code || descriptor.adapter_version !== "3.0.0") fail("notion_role_v3_descriptor_binding_mismatch");
  ...
  if (actor.userUid !== plan.actor_user_uid || actor.authorizationVersion !== plan.actor_authorization_version) fail("notion_role_v3_actor_binding_mismatch");
  ...
  try {
    ...
    const source = await client.query<{ id: string }>("SELECT id::text FROM public.accounting_logical_sources WHERE source_code='NOTION_ORGANIZATION_ROLE_HISTORY' FOR UPDATE"); if (source.rowCount !== 1) fail("notion_role_v3_source_missing");
    const baseline = await client.query<{ v2: number; v3: number; batches: number }>(`SELECT count(*) FILTER (WHERE adapter_version='2.0.0')::int v2,count(*) FILTER (WHERE adapter_version='3.0.0')::int v3,(SELECT count(*)::int FROM public.accounting_import_batches b JOIN public.accounting_source_releases br ON br.id=b.source_release_id WHERE br.logical_source_id=$1) batches FROM public.accounting_source_releases WHERE logical_source_id=$1 AND status='active'`, [source.rows[0].id]);
    if (baseline.rows[0].v2 !== 1 || ![0, 1].includes(baseline.rows[0].v3) || baseline.rows[0].batches !== 0) fail("notion_role_v3_release_baseline_mismatch");
    ...
    if (exact.rowCount === 1) {
      if (exact.rows[0].release_uid !== plan.release_uid) fail("notion_role_v3_existing_release_uid_mismatch");
      const receipt = await client.query("SELECT 1 FROM public.business_operation_receipts WHERE operation_uid=$1::uuid", [plan.operation_uid]); if (receipt.rowCount !== 1) fail("notion_role_v3_receipt_missing");
      await client.query("COMMIT"); return "verified_noop";
    }
    if (baseline.rows[0].v3 !== 0) fail("notion_role_v3_release_drift");
    ...
    const payload = { command: "source_release:create", expected_results: [result], inputs: { descriptor_sha256: plan.descriptor_sha256, source_code: plan.source_code }, reservation_slots: slots, schema_version: "business-operation-payload-v2" }; const payloadSha = sha256(canonicalJson(payload as CanonicalValue)); const now = new Date().toISOString();
    ...
    await client.query(`INSERT INTO public.accounting_audit_events (id,event_uid,entity_type,entity_key,action,before_json,after_json,effective_at,recorded_at,reason_code,actor_user_id,actor_uid_snapshot,actor_name_snapshot,actor_scope,actor_at,correlation_uid,actor_authorization_version) OVERRIDING SYSTEM VALUE VALUES ($1,$2::uuid,'source_release',$3,'create',NULL,$4::jsonb,$5::timestamptz,$5::timestamptz,'TODO18_NOTION_ROLE_NULLABLE_QUARANTINE_V3',$6,$7::uuid,$8,'admin',$5::timestamptz,$9::uuid,$10)`, [auditId, plan.event_uid, result.entity_key, canonicalJson({ descriptor_sha256: plan.descriptor_sha256, source_code: plan.source_code } as CanonicalValue), now, actor.userId, actor.userUid, actor.name, plan.action_correlation_uid, actor.authorizationVersion]);
    await client.query("COMMIT"); return "created";
  } catch (error) { await client.query("ROLLBACK").catch(() => undefined); throw error; }
}

async function main() {
  if (arg("--target") !== "development" || arg("--plan") !== PLAN_PATH || arg("--actor-receipt") !== ACTOR_PATH) fail("notion_role_v3_release_scope_mismatch");
  const resolved = resolveDevelopmentTarget(process.env, "migration"); const pool = createTargetPool(resolved);
  try { const target = await verifyDevelopmentTarget(pool, resolved); const client = await pool.connect(); try { const outcome = await register(client, target.targetFingerprint); console.log(JSON.stringify({ schema_version: "notion-role-history-release-result-v3", target: "development", release_uid: readJson(PLAN_PATH).release_uid, outcome, result: "verified" })); } finally { client.release(); } } finally { await shutdownPool(pool); }
}
main().catch((error) => { console.error(JSON.stringify({ schema_version: "notion-role-history-release-error-v3", error_code: error instanceof Error ? error.message : "unknown", result: "rejected" })); process.exitCode = 1; });
