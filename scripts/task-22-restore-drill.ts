...
async function schemaCatalog(client: PoolClient): Promise<Json> {
  const [relations, columns, constraints, indexes, triggers, routines, types] = await Promise.all([
    ...
    client.query(`SELECT c.relname table_name,a.attnum,a.attname,format_type(a.atttypid,a.atttypmod) data_type,a.attnotnull,pg_get_expr(d.adbin,d.adrelid,true) default_sql,a.attidentity,a.attgenerated FROM pg_attribute a JOIN pg_class c ON c.oid=a.attrelid JOIN pg_namespace n ON n.oid=c.relnamespace LEFT JOIN pg_attrdef d ON d.adrelid=a.attrelid AND d.adnum=a.attnum WHERE n.nspname='public' AND c.relkind IN ('r','p','v','m') AND a.attnum>0 AND NOT a.attisdropped ORDER BY c.relname,a.attnum`),
