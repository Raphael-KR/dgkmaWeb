...
async function runProbe(client: PoolClient): Promise<Record<string, unknown>> {
  ...
  try {
    ...
    const catalog = await client.query<{ schema_constraints: string[]; schema_triggers: string[]; reason_triggers: string[]; payload_not_null: boolean; payload_checks: string[] }>(`SELECT
      ARRAY(SELECT conname FROM pg_catalog.pg_constraint WHERE conrelid='public.schema_data_exceptions'::regclass AND conname LIKE 'schema_data_exceptions__%registry%' OR conrelid='public.schema_data_exceptions'::regclass AND conname IN ('schema_data_exceptions__capture_chain__check','schema_data_exceptions__lifecycle_actor__check','schema_data_exceptions__resolution_duplicate__check','schema_data_exceptions__status_resolution__check') ORDER BY conname) schema_constraints,
      ARRAY(SELECT tgname FROM pg_catalog.pg_trigger WHERE tgrelid='public.schema_data_exceptions'::regclass AND NOT tgisinternal ORDER BY tgname) schema_triggers,
      ARRAY(SELECT tgname FROM pg_catalog.pg_trigger WHERE tgrelid=ANY($1::regclass[]) AND NOT tgisinternal AND tgname LIKE '%business_reason%' ORDER BY tgname) reason_triggers,
      ...
      ARRAY(SELECT conname FROM pg_catalog.pg_constraint WHERE conrelid='public.business_operation_receipts'::regclass AND conname='business_operation_receipts__canonical_payload__check' ORDER BY conname) payload_checks`, [BUSINESS_REASON_TABLES.map((table) => `public.${table}`)]);
    const row = catalog.rows[0];
    if (canonicalJson(row.schema_constraints as never) !== canonicalJson([...SCHEMA_CONSTRAINTS] as never) || canonicalJson(row.schema_triggers as never) !== canonicalJson(["schema_data_exceptions__registry_transition_v1"] as never) || row.reason_triggers.length !== 8 || new Set(row.reason_triggers).size !== 8 || row.payload_not_null !== true || canonicalJson(row.payload_checks as never) !== canonicalJson(["business_operation_receipts__canonical_payload__check"] as never)) throw new Error("task_20_registry_catalog_mismatch");
