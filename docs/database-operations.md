...
Sequence 110은 sequence 100의 immutable manifest/ledger를 보존한 additive business-reason domain 보정이다. 적용 전 8개 표의 비허용 `reason_code` 집계와 동일 constraint 이름 부재를 `REPEATABLE READ READ ONLY`에서 확인한다. disposable `applied → verified_noop`과 catalog 일치 및 teardown `absent:true` 후에만 Development에 적용한다. Development 재실행도 `verified_noop`이어야 한다. Receipt는 `docs/database-targets/development-sequence-110-applied.json`이다.
...
Sequence 130은 역사적 sequence 15 bytes를 변경하지 않고 `schema_data_exceptions`의 정규 rule/status/resolution registry를 additive 적용한다. 적용 전 기존 예외 행이 0이어야 하며, 1건이라도 있으면 `schema_exception_historical_code_migration_required`로 중단해 자동 rename을 금지한다. 적용 후 25개 exact rule, 6개 lifecycle CHECK와 하나의 INSERT/UPDATE/DELETE trigger가 post-capture root·비정상 successor·UPDATE/DELETE를 SQLSTATE `23514`로 거부한다. Disposable 전체 1→130, invalid root, reapply와 teardown을 통과한 뒤에만 Development에 적용한다.

```bash
env -u DATABASE_URL -u PROD_DATABASE_URL -u PROD_DATABASE_READONLY_URL \
  npx tsx scripts/apply-schema.ts --target development --from-sequence 130 --through-sequence 130
```
...
2026-08-11 마지막 Development 검증 상태는 ledger `1,10,15,20,30,40,50,60,70,80,90,100,110,120,130`, approved category tips 6, draft policy/mapping 16/46, approved policy/mapping 0이다. Sequence 130의 schema exception CHECK/trigger 6/1과 예외 행 0, sequence 110/120의 exact reason CHECK/trigger 8/8, legacy v3 zero-row cutover를 확인했다. 관리자 CLI의 다섯 service 작업은 `created → verified_noop`, identity sequence SHA-256 `4b57e8ff…`, receipt collection SHA-256 `c0e35350…`로 불변이었다. 최종 phase는 `new`, watermark 0, legacy release/batch/cutover `3/1/3`, operation receipt/entity/audit `5/6/6`, payment/decision/batch-row/source-decision/classification은 모두 0이다. 실행 receipts는 `docs/database-targets/development-sequence-130-applied.json`, `docs/database-targets/development-sequence-120-applied.json`, `docs/database-targets/development-sequence-110-applied.json`, `docs/database-targets/development-sequence-100-applied.json`과 `docs/database-targets/development-legacy-payment-zero-row-cutover-v1.json`이다. 이후 기존 `payments` write는 DB fence가 거부한다.
