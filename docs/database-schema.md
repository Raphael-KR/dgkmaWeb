...
| Development evidence | verified current state; ledger `[1,10,15,20,30,40,50,60,70]`, sequences 1–60 retain parent manifest `986e515b…`, sequence 70 binds child manifest `31671836…`, catalog SHA `7115c2ceed89f693405303eae6ad826fc0aa98c764774e3f1832b333d385bd37`, identical reapply `verified_noop`, startup ready, catalog transaction terminal `ROLLBACK` |
...
Development의 정본 경로는 checksum-bound ledger sequence `1,10,15,20,30,40,50,60,70`이다. Sequence 40은 `preferred_btree_gist`, sequence 65는 미적용이다. Sequence 70 동일 재실행은 `verified_noop`이었고 migrated startup은 DDL을 방출하지 않았다. `migrations/0000_cheerful_nick_fury.sql`은 sequence 10 생성 입력으로만 유지한다. Production 미검증 상태를 migration drift로 감추지 않는다.
...
체크리스트: ledger `1,10,15,20,30,40,50,60,70`, 68 tables, 1403 columns, 68 PK/257 FK/80 UNIQUE/287 CHECK/432 indexes/63 sequences, routines 188, triggers/views/policies/enums/domains 0, RLS enabled 0, approved category tips 6, approved dues policy/mapping 0, Production unverified를 재확인한다. 재검증은 metadata-only이고 `REPEATABLE READ READ ONLY` 후 `ROLLBACK`하며 행·PII·Secret을 저장하지 않는다.
