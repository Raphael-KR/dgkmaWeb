...
| manifest SHA-256 | `19ac53ce74af375ab5eb6a9c96a3ca4d5fd7cc5127e9ad7bbd1da4cad33ea700` sequence-90 great-grandchild (sequence-80 parent `bf7216af6f30a366c0adad4b355fc6c4bed0aaf625154a70d063db2864d86b3b`; sequence-70 ancestor `31671836f8550190c38f27b15ec3d3e45e330e5fa3c256f3db48cdf059fe64f6`; root `986e515be4055393f13950844bb93dadd1ec1aa2b6484220506ded4f4ce6cef8`) |
...
| Prepared next state | sequence 90 replaces only all-version `legacy_cutover_states(cutover_code)` UNIQUE with the same-name `WHERE version=1` partial unique index; disposable `applied → verified_noop`, same-code v1/v2 insertion+`ROLLBACK`, catalog `ROLLBACK`, teardown absence passed; Development remains through sequence 80 pending approval |
...
Development의 정본 경로는 checksum-bound ledger sequence `1,10,15,20,30,40,50,60,70,80`이다. Sequence 40은 `preferred_btree_gist`, sequence 65는 미적용이다. Sequence 80 동일 재실행은 `verified_noop`이었고 migrated startup은 DDL을 방출하지 않았다. `migrations/0000_cheerful_nick_fury.sql`은 sequence 10 생성 입력으로만 유지한다. Production 미검증 상태를 migration drift로 감추지 않는다.
