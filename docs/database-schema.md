...
| Prepared next state | sequence 90 replaces only all-version `legacy_cutover_states(cutover_code)` UNIQUE with the same-name `WHERE version=1` partial unique index; repository/disposable verification precedes any Development apply |
