...
- Todo 18 source coverage는 종료됐다. Todo 19의 legacy `payments` preflight와 v3 adapter, cutover VCHAIN을 여는 additive sequence 90의 repository/disposable/Development 적용까지 완료됐다. 다음 실행 경로는 immutable legacy v3 release와 zero-row batch evidence를 등록한 뒤, 관리자 CLI와 동일 application service 경로로 zero-row `legacy→fenced→new` cutover를 구현·검증하는 것이다.
