...
| Todo 20 현재 단위 | strict payload/live-service, 48개 reason, 25-code exception, KST rights·close/balance·62-class lock trace 완료. `cfb17a0` registry disposable, `2d8d445` stored payload/hash/result·same-UID replay sequence 불변에 이어 `b91f9fd`가 서로 다른 operation UID의 동일 source-decision key race에서 1 commit/1 serialization abort·loser receipt 0·DB absence를 증명; refund/annual activation과 나머지 workflow 전체 harness가 남음 |
