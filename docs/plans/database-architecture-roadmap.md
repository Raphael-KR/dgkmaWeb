...
| manifest | sequence-100 descendant SHA-256 `551d9d672a0cd3689b6c3ac5d1252078f4e53106a7ef143ac954c96239596c14` prepared |
| Development schema | ledger `[1,10,15,20,30,40,50,60,70,80,90]`; sequence 90 `applied → verified_noop`; startup/catalog 승인 |
...
| Todo 19 DB write fence | sequence 100 migration·manifest·startup gate·focused tests 구현 완료; disposable와 Development 적용은 다음 단계 |
...
- Todo 18 source coverage는 종료됐다. Todo 19의 legacy `payments` preflight와 v3 adapter, cutover VCHAIN을 여는 additive sequence 90의 repository/disposable/Development 적용까지 완료됐다. 애플리케이션 잠금만으로는 cutover 후 다른 write path를 막지 못한다는 gap을 확인해 sequence 100 DB-resident write fence를 구현했다. 다음 실행 경로는 disposable에서 sequence 100과 zero-row `legacy→fenced→new`를 검증한 뒤 Development에 동일 순서로 적용하는 것이다.
