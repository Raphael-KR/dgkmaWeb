...
| Todo 20 현재 단위 | strict payload/live-service, 48개 reason, 25-code exception, KST rights·close/balance·62-class lock trace 완료. registry disposable, stored payload/hash/result·same-UID replay sequence 불변, 서로 다른 operation UID의 동일 stable-key race `1 commit/1 serialization abort`·loser receipt 0·DB absence를 증명했다. `1d08feb`/`1a75a76`은 ordinary·두 correction refund와 6-tier policy-before-mapping activation/atomic rollback 계약을 추가했고 Replit `npm run check` 및 집중 테스트 16/16을 통과했다. refund/annual activation의 실제 DB application-service 경계와 나머지 workflow 전체 harness가 남음 |
